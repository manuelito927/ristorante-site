(function () {
  ...
  // CREA nuovo prodotto (POST /admin/menu)
  if (createBtn) {
    createBtn.onclick = async () => {
      const newCard = createBtn.closest(".card");
    ...
    try {
      ...
      (items || []).forEach((it) => {
        ...
        wrap.querySelector('[data-act="del"]').onclick = async () => {
          ...
          if (!ok) return;
          try {
            await api("/admin/menu/" + it.id, { method: "DELETE" });
            openId = null;
            loadItems();
          } catch (e) {
            alert("❌ " + e.message);
  ...
  async function loadComeFunziona() {
    try {
      const res = await api("/page/come-funziona");
  ...
  async function saveComeFunziona() {
    ...
    await api("/admin/page/come-funziona", {
      method: "PUT",
      body: JSON.stringify(payload)
    });
  ...
  async function loadReservations() {
    ...
    try {
      const { reservations } = await api("/admin/reservations?limit=50");
