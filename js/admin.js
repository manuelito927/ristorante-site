...
function renderStripItemsEditor(items) {
  ...
  items.forEach((it, idx) => {
    ...
    row.innerHTML = `
      <button data-s="toggle" style="all:unset; width:100%; cursor:pointer;">
    ...
    toggleBtn.onclick = () => {
      ...
      stripItemsGrid.querySelectorAll('[data-s="editor"]').forEach(e => e.hidden = true);
    ...
    row.querySelector('[data-s="save"]').onclick = async () => {
      ...
      try {
        ...
        await saveStripData(key, title, order, next);
